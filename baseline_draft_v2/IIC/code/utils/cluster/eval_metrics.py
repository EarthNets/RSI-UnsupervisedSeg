...
def _hungarian_match(flat_preds, flat_targets, preds_k, targets_k):
  ...
  for i in range(len(match[0])):
    res.append((torch.tensor(match[0][i].astype(np.uint8)), torch.tensor(match[1][i].astype(np.uint8))))
...
def _iou(preds, targets, num_k, verbose=0):
    iou = []
    for i in range(num_k):
        pred = (preds==i)
        target = (targets==i)
        iou.append((pred & target).sum() / float((pred | target).sum()))
    return iou
