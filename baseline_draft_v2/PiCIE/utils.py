...
from data.rgbd_train_dataset import TrainRGBD
from data.rgbd_eval_dataset import EvalRGBD
...
def get_dataset(args, mode, inv_list=[], eqv_list=[]):
    if args.cityscapes:
        ...
        if mode == 'train':
            dataset = TrainRGBD(args.data_root, labeldir=args.save_model_path, res1=args.res1, res2=args.res2,
                                split='train', mode='compute', inv_list=inv_list, eqv_list=eqv_list, scale=(args.min_scale, 1), include_height=False)
        elif mode == 'train_val':
            dataset = EvalRGBD(args.data_root, res=args.res, split='val', mode='test',
                                     label_mode=args.label_mode, long_image=args.long_image, include_height=False)
        elif mode == 'eval_val':
            dataset = EvalRGBD(args.data_root, res=args.res, split=args.val_type, 
                                     mode='test', label_mode=args.label_mode, long_image=args.long_image, label=False, include_height=False)
        elif mode == 'eval_test':
            dataset = EvalRGBD(args.data_root, res=args.res, split='test', mode='test',
                                     label_mode=args.label_mode, long_image=args.long_image, include_height=False)            
    elif args.rgbd:
        if mode == 'train':
            dataset = TrainRGBD(args.data_root, labeldir=args.save_model_path, res1=args.res1, res2=args.res2,
                                split='train', mode='compute', inv_list=inv_list, eqv_list=eqv_list, scale=(args.min_scale, 1), include_height=True)
        elif mode == 'train_val':
            dataset = EvalRGBD(args.data_root, res=args.res, split='val', mode='test',
                                     label_mode=args.label_mode, long_image=args.long_image, include_height=True)
        elif mode == 'eval_val':
            dataset = EvalRGBD(args.data_root, res=args.res, split=args.val_type, 
                                     mode='test', label_mode=args.label_mode, long_image=args.long_image, label=False, include_height=True)
        elif mode == 'eval_test':
            dataset = EvalRGBD(args.data_root, res=args.res, split='test', mode='test',
                                     label_mode=args.label_mode, long_image=args.long_image, include_height=True)        
