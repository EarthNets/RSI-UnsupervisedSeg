...
from code.datasets.segmentation import rgbd,rgb
import pdb
...
def segmentation_create_dataloaders(config):
  if config.mode == "IID+":
    ...
    elif config.dataset in ["DFC", "DC"]:
      config.train_partitions = ["unlabelled_train"]
      config.mapping_assignment_partitions = ["labeled_train"]
      config.mapping_test_partitions = ["labelled_test"]
    elif config.dataset=='RGB':
      config.train_partitions = ["unlabelled_train"]
      config.mapping_assignment_partitions = ["labeled_train"]
      config.mapping_test_partitions = ["labelled_test"]
    elif config.dataset=='RGBD':
      config.train_partitions = ["unlabelled_train"]
      config.mapping_assignment_partitions = ["labeled_train"]
      config.mapping_test_partitions = ["labelled_test"]
    else:
    ...
    if "Coco10k" in config.dataset:
      ...
      config.train_partitions = ["unlabelled_train"]
      config.mapping_assignment_partitions = ["labelled_test"]
      config.mapping_test_partitions = ["labelled_test"]
    elif config.dataset == 'RGBD':
      config.train_partitions = ["unlabelled_train"]
      config.mapping_assignment_partitions = ["labelled_test"]
  ...
  if "Coco" in config.dataset:
    ...
    dataloaders, mapping_assignment_dataloader, mapping_test_dataloader = \
      make_DFC_dataloaders(config)
  elif config.dataset == "DC":
    dataloaders, mapping_assignment_dataloader, mapping_test_dataloader = \
      make_DC_dataloaders(config)
  elif config.dataset == "RGBD":
    dataloaders, mapping_assignment_dataloader, mapping_test_dataloader = \
      make_RGBD_dataloaders(config)
  elif config.dataset == "RGB":
    dataloaders, mapping_assignment_dataloader, mapping_test_dataloader = \
      make_RGB_dataloaders(config)      
  else:
    raise NotImplementedError
...
def make_DFC_dataloaders(config):
  dataloaders = _create_dataloaders(config, dfc.__dict__[config.dataset])

  mapping_assignment_dataloader = \
    _create_mapping_loader(config, dfc.__dict__[config.dataset],
                          partitions=config.mapping_assignment_partitions)
  ...
  return dataloaders, mapping_assignment_dataloader, mapping_test_dataloader
def make_DC_dataloaders(config):
  dataloaders = _create_dataloaders(config, dc.__dict__[config.dataset])

  mapping_assignment_dataloader=\
    _create_mapping_loader(config, dc.__dict__[config.dataset],
                           partitions=config.mapping_test_partitions)
  mapping_test_dataloader = \
    _create_mapping_loader(config, dc.__dict__[config.dataset],
                          partitions=config.mapping_test_partitions)
...
def make_RGBD_dataloaders(config):
  dataloaders = _create_dataloaders(config, rgbd.__dict__[config.dataset])

  mapping_assignment_dataloader=\
    _create_mapping_loader(config, rgbd.__dict__[config.dataset],
                           partitions=config.mapping_test_partitions)
  mapping_test_dataloader = \
    _create_mapping_loader(config, rgbd.__dict__[config.dataset],
                          partitions=config.mapping_test_partitions)
...
def make_RGB_dataloaders(config):
  dataloaders = _create_dataloaders(config, rgb.__dict__[config.dataset])

  mapping_assignment_dataloader=\
    _create_mapping_loader(config, rgb.__dict__[config.dataset],
                           partitions=config.mapping_test_partitions)
  mapping_test_dataloader = \
    _create_mapping_loader(config, rgb.__dict__[config.dataset],
                          partitions=config.mapping_test_partitions)
...
def _create_dataloaders(config, dataset_class):
  ...
  for d_i in range(config.num_dataloaders):
    ...
    #print(config.train_partitions,len(train_imgs_curr),len(train_imgs))
