...
from .eval_metrics import _hungarian_match, _original_match, _acc, _iou
...
def cluster_subheads_eval(config, net,
  ...
  all_matches, train_accs, train_ious = _get_assignment_data_matches(net,
                                                         mapping_assignment_dataloader,
                                                         config,
                                                         sobel=sobel,
                                                         using_IR=using_IR,
                                                         ...
                                                         verbose=verbose)
  #print("all_matches", all_matches.device, "train_accs", train_accs.device)
  ...
  return {"test_accs": list(test_accs),
          "test_ious": list(test_ious),
          "avg_acc": np.mean(test_accs),
          "avg_iou": np.mean(np.array(test_ious)),
...
def _get_assignment_data_matches(net, mapping_assignment_dataloader, config,
  ...
  if not just_matches:
    ...
    all_ious = np.zeros(config.num_sub_heads, dtype=np.float32)
  for i in range(config.num_sub_heads):
    if verbose:
    ...
    if not just_matches:
      ...
      iou = _iou(reordered_preds, flat_targets_all, config.gt_k, verbose)
      all_accs[i] = acc
      all_ious[i] = np.array(iou).mean()

  if just_matches:
    return all_matches
  else:
    return all_matches, all_accs, all_ious
