...
class TrainRGBD(data.Dataset):
    def __init__(self, root, labeldir, mode, split='train', res1=1024, res2=1024, inv_list=[], eqv_list=[], scale=(0.5, 1), include_height=False):
        self.root  = root 
        self.split = split
        self.res1  = res1
        self.res2  = res2  
    ...
    def load_imdb(self):
        imdb = []
        for fname in os.listdir(os.path.join(self.root, 'images', 'training')):
            image_path = os.path.join(self.root, 'images', 'training', fname)
            
            ### height ###
            if self.include_height:
                height_path = os.path.join(self.root, 'heights', 'training', fname.replace('RGB', 'AGL'))
            else:
                height_path = None
            ##############            
                        
            imdb.append((image_path,height_path))
    ...
    def __getitem__(self, index):
        ...
        img_path,h_path = self.imdb[index]
        image = self.load_data(img_path)
        if self.include_height:
            height_np = rasterio.open(h_path).read(1)
        ...
        image = self.transform_image(index, image, height)
    ...
    def transform_image(self, index, image, height):
        # Base transform: resize, crop
        image = self.transform_base(index, image)
        if self.include_height:
            height = self.transform_base(index, height)

        if self.mode == 'compute':
            if self.view == 1:
                image = self.transform_inv(index, image, 0) # color distortion
                image = self.transform_tensor(image)
                if self.include_height:
                    height = transforms.ToTensor()(height)
                    height = transforms.Normalize(mean=[4.79], std=[61.67])(height)
                    image = torch.cat((image,height),0)
                
            elif self.view == 2:
                image = self.transform_inv(index, image, 1) # color distortion
                image = TF.resize(image, self.res1, Image.BILINEAR)
                image = self.transform_tensor(image)
                if self.include_height:
                    height = TF.resize(height, self.res1, Image.BILINEAR)
                    height = transforms.ToTensor()(height)
                    height = transforms.Normalize(mean=[4.79], std=[61.67])(height)
                    image = torch.cat((image,height),0)
            else:
            ...
            if self.include_height:
                height1 = transforms.ToTensor()(height)
                height1 = transforms.Normalize(mean=[4.79], std=[61.67])(height1)
                image1 = torch.cat((image1,height1),0)
            ...
            if self.include_height:
                height2 = TF.resize(height, self.res1, Image.BILINEAR)
                height2 = transforms.ToTensor()(height2)
                height2 = transforms.Normalize(mean=[4.79], std=[61.67])(height2)
                image2 = torch.cat((image2,height2),0)            
